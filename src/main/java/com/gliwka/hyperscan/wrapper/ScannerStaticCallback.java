 ...
 */
public class ScannerStaticCallback implements Closeable {
    private static final AtomicInteger scannerId = new AtomicInteger();
    private static final ConcurrentHashMap<Integer, ScannerStaticCallback> scannerPool = new ConcurrentHashMap<>();
    private final Integer scannerIdx = scannerId.incrementAndGet();
    private final IntPointer scannerInstanceId = new IntPointer(1);

    public ScannerStaticCallback() {
    ...
    // Set to track databases for which scratch space has been allocated on this scanner instance.
    private final Set<Database> allocatedDatabases = new HashSet<>();
     ...
     * database that will be used before scan is called. If scratch space
     * has already been allocated for the given Database on this Scanner instance,
    ...
    public void allocScratch(final Database db) {
        ...
        // Check if already allocated for this DB on this scanner
        if (allocatedDatabases.contains(db)) {
            return; // No-op
        }
        ...
        // Allocation successful, register deallocator and add DB to set
        scratch.registerDeallocator();
        allocatedDatabases.add(db);
    }
    ...
    private static final match_event_handler matchHandler = new match_event_handler() {
        public int call(int id, long from, long to, int flags, Pointer context) {
            try(IntPointer intContext = new IntPointer(context)) {
                int scannerId = intContext.get();
                ScannerStaticCallback scanner = scannerPool.get(scannerId);
    ...
    public void close() {
        ...
        allocatedDatabases.clear(); // Clear the set on close
    }
}
